...
import { WsFriendConnection, WsFriendDisconnection, WsLeftFriend, WsNewFriend } from '@/shared/WsEvents/friend';
import {Socket} from 'socket.io-client';

export function setUpFriendEvents(socket: Socket, dispatch: AppDispatch): void {
  socket.on(
    WsNewFriend.eventName,
    (message: WsNewFriend.eventMessageTemplate) => {
  ...
  socket.on(
    WsLeftFriend.eventName,
    (message: WsLeftFriend.eventMessageTemplate) => {
      const {friendId} = message;
  ...
  socket.on(
    WsFriendConnection.eventName,
    (message: WsFriendConnection.eventMessageTemplate) => {
      const {friendId} = message;
      dispatch(refreshInvitation('FRIEND'));
  ...
  socket.on(WsFriendDisconnection.eventName, (message: WsFriendDisconnection.eventMessageTemplate) => {
    const {friendId} = message;
    dispatch(refreshInvitation('FRIEND'));
