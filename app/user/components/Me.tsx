...
const Me = () => {
    ...
    return (
        <VStack 
            ...
            mx="auto"
        >
            {data.avatarUrl && <Image borderRadius="full" boxSize="150px" src={data.avatarUrl} alt="Avatar" />}
            <Text fontSize="xl" fontWeight="bold">{data.nickname}</Text>
            <Text>{data.email}</Text>
            <Text>{data.userId}</Text>
			<Link href="/user/edit-profile" passHref>
                <Button as="a" colorScheme="teal" size="sm">Edit Profile</Button>
            </Link>
        </VStack>
    )
}
